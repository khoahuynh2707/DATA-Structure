...
public class Main {
    private static final java.util.List<Book> bookList = new ArrayList<>();
    ...
    try {
        ...
        searchBtn.addActionListener(e -> {
            ...
            if (found != null) {
                ...
                orderInfoArea.setText("Order with Order ID not found: " + searchId);
            }
        });

        // Order deletion event
        deleteBtn.addActionListener(e -> {
            String deleteId = searchOrderIdField.getText().trim();
            if (deleteId.isEmpty()) {
                JOptionPane.showMessageDialog(frame, "Enter Order ID to delete!");
                return;
            }
            ...
            if (removed) {
                ...
                orderInfoArea.setText(sb.toString());
                JOptionPane.showMessageDialog(frame, "Deleted order with Order ID: " + deleteId);
            } else {
                JOptionPane.showMessageDialog(frame, "Order with Order ID not found: " + deleteId);
            }
        });

        JButton undoAddBookBtn = new JButton("Undo Add Book");
        undoAddBookBtn.addActionListener(e -> {
            if (!bookHistory.isEmpty()) {
                Book lastAddedBook = bookHistory.pop();
        ...
        addBookPanel.add(undoAddBookBtn, BorderLayout.SOUTH);
